...
public void preInit(FMLPreInitializationEvent event)
{
     ...
     {
         FMLLog.log(Level.SEVERE, e, "AEPeripheral has a problem loading it's configuration");
     }
     finally
     {
         cfg.save();
...
public void load(FMLInitializationEvent evt)
{
	AEPeripheralblock = new BlockAEPeripheral(blockId);
	 GameRegistry.registerBlock(this.AEPeripheralblock, "AEPeripheral.BlockAEPeripheral");
	 LanguageRegistry.instance().addNameForObject(AEPeripheralblock, "en_US", "AE Peripheral");
	 GameRegistry.registerTileEntity(TileEntityAEPeripheral.class, "AEPeripheral");
	 
	 AEPeripheralGenericTick_server = new AEPeripheralTickHandler();
    TickRegistry.registerTickHandler(AEPeripheralGenericTick_server, Side.SERVER);
    MinecraftForge.EVENT_BUS.register(AEPeripheralGenericTick_server);
	 ...
	 if(AEPeripheralUtil.isClient())
	 {
	     ...
	     TickRegistry.registerTickHandler(AEPeripheralGenericTick_client, Side.CLIENT);
	     MinecraftForge.EVENT_BUS.register(AEPeripheralGenericTick_client);
	 }
	 
	 ShapedOreRecipe oreRecipe = new ShapedOreRecipe(new ItemStack(AEPeripheralblock), new Object[]{
             "AAA","BCA", "AAA", 'A', Block.stone,'B', Blocks.blkColorlessCable, 'C', Item.redstone });
